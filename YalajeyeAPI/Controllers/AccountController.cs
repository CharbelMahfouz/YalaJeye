...
namespace YalajeyeAPI.Controllers
{
    ...
    public class AccountController : ControllerBase
    {
        ...
        public AccountController(UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, SignInManager<ApplicationUser> signInManager, IUnitOfWork unit)
        {
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<dynamic> Register([FromBody] Register_VM model)
        {
            ...
            var user = new ApplicationUser { FirstName = model.FirstName, LastName= model.LastName, UserName = model.Email, Email = model.Email, PhoneNumber = model.PhoneNumber, Image = model.PhotoURL };
            ...
            var createdUser = await _unit.CustomerRepository.Create(userObj);
            Customer_VM customervm = _unit.Mapper.Map<Customer_VM>(createdUser);
            customervm.token = JwtToken;
            return customervm;
        ...
        [HttpPost]
        [AllowAnonymous]
        public async Task<dynamic> Login(Login_VM model)
        {
            ...
            var customer =  _unit.CustomerRepository.GetByIdWithInclude(x => x.UserId == user.Id);
            Customer_VM customervm = _unit.Mapper.Map<Customer_VM>(customer);
            customervm.token = handlerSerialized;
            return customervm;
        }
    }
}
